...
每个 task-NNN.prompt.txt 原样喂给外部工具（不要改写、不要摘要），回执格式见下。这些通道本来就没有 CC 子代理的固定开销，成本按各家额度自行折算。agy 注意：无结构化输出、退出码不可信，提示词里要求"只输出这个 JSON、不要任何别的话"，**并声明"不得使用任何工具/命令"**——headless 下模型偶发想调工具，会被自动拒绝并产出空 stdout（2026-07-19 实测，rc 仍为 0，报错只在 stderr）；回来自行解析校验。
