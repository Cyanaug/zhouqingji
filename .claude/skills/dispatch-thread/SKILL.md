...
python plan_thread.py invite --parent <长评的read_id> --fraction 0.5 \
    --out ../runners/batches/thread-<read_id>-<批次标识>

# 后续层：对某条已有楼层继续接楼
python plan_thread.py invite --parent <子楼的read_id> --fraction 0.5 \
    --out ../runners/batches/thread-<root_id>-r<N>
```
...
```
python plan_thread.py collect --tasks ../runners/batches/<批次>/tasks \
    --inbox ../runners/batches/<批次>/inbox --model <模型ID>
```

collect 打印：落盘条数、沉默条数、引用校验拒绝条数、顺势投票条数。
