...
用 Agent 工具，`subagent_type: "task-runner"`。模型按批次/档位指定；并发数按当前
Claude Code 配置、额度与限流分波，不在项目文档里写死：
...
每个 task-NNN.prompt.txt 原样喂给外部工具（不要改写、不要摘要），每个 task
启动一个全新进程。当前 AGY 与 CodeBuddy 均支持 JSON/JSON schema 输出；优先使用
当前 CLI 的结构化输出和工具禁用选项，同时仍须检查 stdout、stderr、JSON 与必填字段，
不能只信退出码。CLI 能力会升级，派发前以本机 `--help` 为准。
