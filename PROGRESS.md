...
## 2026-07-18 · v1.5 机制审查落地：加精 + 派发降本 + 跟帖分层

机制审查发现三处设计意图与实测脱节（主动票 up 占八成、跟帖 99% 停在一楼、点赞/跟帖走全工具子代理贵通道），四个包一次落地：

- **点赞加精（best）**：批量投票回执新增 `best`——同箱横向比较里"最扛得住的一条"。逐条 up/down/skip 是绝对判断、有正向偏置；加精是相对判断、不受偏置影响。vote 值域扩展 `best`（旧统计自动忽略）；webapp ⭐N 徽记（`.vbest`）；方向票索引排除 best 防覆盖。
- **点赞降本**：批量装箱改字符预算（`--batch-size 8` + `--batch-chars 4000` 双闸）；选人改逐投票人裁剪选票。新建通用轻量子代理 `task-runner`（Read+Write，跟帖/点赞通用，开销约为全工具子代理 1/4；需新会话生效）。外部 CLI 通道并列不预设。
- **跟帖分层 + nextround**：广度档（一楼 0.5、便宜通道）/深度档（2 级起 ~0.25、更强通道），只谈贵贱不点名模型。一楼 prompt 裁掉立场惯性段与 stance 字段；优先回应权=楼主+被回击者，parent 作者不自回；invite 默认去重已回帖者（`--allow-repeat` 放开）。新增 `plan_thread.py nextround`：按争议度/立场撼动/楼主下场自动挑候选楼层、打印现成 invite 命令——只提案不派发。
- **诗正文 + 小修**：跟帖 prompt 补诗全文（此前只给标题是疏漏）；`cmd_ingest` 改 all-or-nothing 并返回新记录（修孤儿楼层与重复入库隐患）；plan_votes help 文案改正。

测试 ALL PASS + 真实数据冒烟（invite 各档 prompt 内容、nextround 候选、best 脏值解析）。
