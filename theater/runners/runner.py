...
DATA_LOCK = ROOT / "results" / ".write.lock"
...
@contextmanager
def data_lock(timeout=30):
    """跨线程、跨进程串行化 results 写入；同一线程可重入。

    JSONL 的 ID 分配依赖“读取现状 -> 算下一个 ID -> 追加”是一个整体。只给
    append 加锁仍会让两个 collector 分到相同 ID，因此所有相关步骤共用一把锁。
    锁文件只作协调，不承载业务数据，可长期保留。
    """
            ...
            while not acquired:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise TimeoutError(f"等待数据写锁超时：{DATA_LOCK}")
                    time.sleep(0.05)
            _LOCK_STATE.depth = 1
            yield
        finally:
...
def vote_identity(vote):
    """一名读者对一个目标、在一个信号通道里的唯一票据身份。"""
    voter = vote.get("voter") or {}
    value = vote.get("vote")
    if value == "best":
        # best 是“这一张票面里最好的一条”，换一箱候选可以再次选中同一评论。
        # 新记录用 ballot_id 对重试幂等；旧记录没有批次来源，只能用原有目标回退。
        ballot = vote.get("ballot_id")
        return ("best", "ballot" if ballot else "legacy",
                ballot or vote.get("target_read_id"), voter.get("persona_id"))
    source = "piggyback" if vote.get("source") == "piggyback" else "direct"
    return "directional", source, vote.get("target_read_id"), voter.get("persona_id")


def _accepted_comment_votes(existing, void, new_votes):
    active = {vote_identity(v): v for v in existing
              if v.get("vote_id") not in void}
    accepted = {}
    allowed = {"up", "down", "skip", "best"}
    for i, v in enumerate(new_votes):
        key = vote_identity(v)
        if v.get("vote") not in allowed or not key[2] or not key[3]:
            raise ValueError(f"第 {i + 1} 张票缺少合法 vote/target/voter：{v!r}")
        old = accepted.get(key) or active.get(key)
        if old:
            same_value = old.get("vote") == v.get("vote")
            same_best_target = (v.get("vote") != "best" or
                                old.get("target_read_id") == v.get("target_read_id"))
            if not (same_value and same_best_target):
                raise ValueError(
                    f"同一投票身份出现冲突：{key}，"
                    f"已有 {old.get('vote')}@{old.get('target_read_id')}，"
                    f"新值 {v.get('vote')}@{v.get('target_read_id')}"
                )
            continue
        accepted[key] = v
...
def validate_comment_votes(new_votes):
    """只做与 append 完全相同的幂等/冲突预检，不写文件。"""
    with data_lock():
        return len(_accepted_comment_votes(load_comment_votes(), load_vote_void(), new_votes))


def append_comment_votes(new_votes):
    """幂等追加票据；同一身份重试跳过，方向冲突则整批拒收。

    主动票与跟帖顺势票属于不同信号通道，互不去重。model 是执行出处，不是
    投票人身份；同一 persona 换模型重跑也不能变成第二张独立票。
    """
    with data_lock():
        existing = load_comment_votes()
        void = load_vote_void()
        accepted = _accepted_comment_votes(existing, void, new_votes)
...
def main():
    ...
    k.add_argument("--transport", required=True,
                   help="真实执行通道，如 codex-subagent/cc-subagent/agy-subagent/codebuddy-cli")
