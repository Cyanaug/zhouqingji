...
一个贯穿三档的试金石：**把这条评论原样搬到另一首诗下面，还成不成立？** 越是"换首诗照样能说"的话，越空；越是"只有对着这首诗才说得出"的话，越实。

- up：论证咬住了这首诗独有的东西，具体、站得住——你愿意拿它去替这个分辩护。投 up 前必须过一关：评论里有没有一句你愿意逐字引用、当作这个分的依据的话？有，把那句原话抄进 reason；一句都抄不出来，就不是 up。
- skip：不算错，但空泛——换首诗也照样能讲，没真正咬住这一首。多数四平八稳的评论就落在这档，选它是诚实的判断，不是失职。不要因为"总得表个态"就把该 skip 的抬成 up。
- down：论证没撑住结论——分数与理由脱节、说的对不上诗本身、或通篇套话。down 时在 reason 里点名是哪一句露了馅。
...
三档含义见上（试金石：换首诗还成不成立）：up 必须能逐字抄出当依据的原句，抄不出就降 skip；skip 是空泛但不错的常态档；down 要点名换首诗照样成立、或对不上诗的那句。
逐条独立判断，不要给整批一个统一态度——同一批里出现清一色 up 几乎一定是偷懒；read_id 必须原样照抄，不能省略或改写。"""
