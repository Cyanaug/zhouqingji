...
def cmd_collect(args):
    ...
    if ingested_lines:
        ...
        # reads、thread meta、顺势票共用同一把进程锁，避免两个 collector
        # 同时分配 ID 或用旧 meta 覆盖彼此。文件仍各自保持 append/原子替换语义。
        with R.data_lock():
            piggyback_votes = []
            for incoming, m in zip(ingested_lines, metas):
                if m.get("vote") in ("up", "down"):
                    piggyback_votes.append({
                        "poem_id": incoming["poem_id"],
                        "target_read_id": m["parent_read_id"],
                        "voter": {"persona_id": incoming["reader"]["persona_id"],
                                  "model": incoming["reader"]["model"]},
                        "vote": m["vote"],
                        "reason": "",
                        "source": "piggyback",
                    })
            # 在 reads/meta 写入前完成冲突预检；同一锁内状态不会变化。
            R.validate_comment_votes(piggyback_votes)
...
def main():
    ...
    c.add_argument("--transport", required=True,
