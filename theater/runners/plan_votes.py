...
def cmd_collect(args):
    ...
    for tf in sorted(tdir.glob("task-*.json")):
        ...
        if "targets" in t:
            ...
            task_votes, bad, seen_rids = [], [], set()
            for v in votes_list:
                rid = v.get("read_id", "")
                vote = v.get("vote")
                if (rid not in valid_rids or rid in seen_rids or
                        vote not in ("up", "down", "skip")):
            ...
            omitted = sorted(valid_rids - seen_rids)
            if bad or omitted:
                print(f"  {tf.stem}: 票面无效（坏值/重复={bad}，缺失={omitted}），整份拒收",
                      file=sys.stderr)
                missing.append(tf.stem + "（batch 票面不完整）")
                continue
