...
def test_thread_vote_conflict_preflight_keeps_read_and_response():
    poem = _setup_corpus()
    root = {"read_id": "r-800", "poem_id": poem["id"],
            "reader": {"persona_id": "op", "model": "m"},
            "context_mode": "blind", "thread_ref": None, "transport": "test",
            "score": 8.0, "reaction": "短评", "long_form": "灯火仍在。",
            "content_hash": poem["content_hash"], "ts": "2026-01-01T00:00:00"}
    _write_reads([root], "reads_conflict.jsonl")
    R.DATA_LOCK = TMP / ".write.lock"
    R.THREAD_DIR = TMP
    R.THREAD_META = TMP / "thread_meta_conflict.json"
    R.THREAD_SILENCES = TMP / "silences_conflict.jsonl"
    R.VOTES_DIR = TMP
    R.VOTES = TMP / "votes_thread_conflict.jsonl"
    R.VOTES_VOID = TMP / "void_thread_conflict.json"
    for p in (R.THREAD_META, R.THREAD_SILENCES, R.VOTES, R.VOTES_VOID):
        if p.exists():
            p.unlink()
    R.append_comment_votes([{
        "poem_id": poem["id"], "target_read_id": root["read_id"],
        "voter": {"persona_id": "reader1", "model": "old"},
        "vote": "up", "reason": "", "source": "piggyback",
    }])

    tdir, idir = TMP / "thread_conflict_tasks", TMP / "thread_conflict_inbox"
    for d in (tdir, idir):
        if d.exists():
            shutil.rmtree(d)
        d.mkdir(parents=True)
    task = {
        "poem_id": poem["id"], "parent_read_id": root["read_id"],
        "root_id": root["read_id"], "parent_text": root["long_form"],
        "persona_id": "reader1", "persona_hash": "h", "depth": 1,
        "reader": {"knows_诠释": False, "knows_date": False},
        "content_hash": poem["content_hash"], "prompt": "...",
    }
    ...
    response.write_text(json.dumps({
        "model": "new", "quote": "灯火仍在。", "reaction": "我仍不同意。",
        "long_form": None, "vote": "down",
    }, ensure_ascii=False), encoding="utf-8")
    ...
    try:
        PT.cmd_collect(_A)
        assert False, "冲突顺势票必须在 read 落盘前拒收"
    except ValueError:
        pass
    assert len(R.load_reads()) == 1, "预检失败不能产生孤儿 thread read"
    assert response.exists(), "预检失败不能提前归档回执"
    print("[ok] thread 顺势票冲突在 read 落盘前拒收")
...
if __name__ == "__main__":
    ...
    test_thread_vote_conflict_preflight_keeps_read_and_response()
