...
def test_append_and_tally():
    ...
    R.VOTES_VOID = TMP / "void_a.json"
    R.DATA_LOCK = TMP / ".write.lock"
    if R.VOTES.exists():
        R.VOTES.unlink()
    if R.VOTES_VOID.exists():
        R.VOTES_VOID.unlink()
...
def test_append_is_idempotent_and_conflicts_are_atomic():
    R.VOTES_DIR = TMP
    R.VOTES = TMP / "votes_idempotent.jsonl"
    R.VOTES_VOID = TMP / "void_idempotent.json"
    R.DATA_LOCK = TMP / ".write.lock"
    for p in (R.VOTES, R.VOTES_VOID):
        if p.exists():
            p.unlink()

    direct = {"poem_id": "zq-test", "target_read_id": "r-9",
              "voter": {"persona_id": "reader-a", "model": "model-1"},
              "vote": "up", "reason": "第一次"}
    assert R.append_comment_votes([direct]) == 1

    retry_other_model = {**direct,
                         "voter": {"persona_id": "reader-a", "model": "model-2"},
                         "reason": "重试时理由措辞变化"}
    assert R.append_comment_votes([retry_other_model]) == 0, \
        "同一 persona 换模型重试也不能多算一票"

    piggy = {**direct, "source": "piggyback"}
    assert R.append_comment_votes([piggy]) == 1, \
        "主动票和跟帖顺势票是两条独立信号"

    conflict = {**direct, "vote": "down"}
    before = R.VOTES.read_text(encoding="utf-8")
    try:
        R.append_comment_votes([
            {**direct, "target_read_id": "r-10"},
            conflict,
        ])
        assert False, "同一身份方向冲突必须拒收"
    except ValueError:
        pass
    assert R.VOTES.read_text(encoding="utf-8") == before, \
        "冲突批次必须 all-or-nothing，前面的合法票也不能先落盘"

    best_a = {**direct, "target_read_id": "r-11", "vote": "best",
              "ballot_id": "batch-a"}
    assert R.append_comment_votes([best_a]) == 1
    try:
        R.append_comment_votes([{**best_a, "target_read_id": "r-12"}])
        assert False, "同一票面不能出现两个不同 best"
    except ValueError:
        pass
    assert R.append_comment_votes([{**best_a, "ballot_id": "batch-b"}]) == 1, \
        "不同票面可以再次把同一评论选为 best"
    print("[ok] 投票幂等 / 主动与顺势分流 / 冲突整批拒收")
...
def test_collect_conflict_keeps_response():
    poem = _setup_corpus()
    R.VOTES_DIR = TMP
    R.VOTES = TMP / "votes_collect_conflict.jsonl"
    R.VOTES_VOID = TMP / "void_collect_conflict.json"
    R.DATA_LOCK = TMP / ".write.lock"
    for p in (R.VOTES, R.VOTES_VOID):
        if p.exists():
            p.unlink()
    R.append_comment_votes([{
        "poem_id": poem["id"], "target_read_id": "r-600",
        "voter": {"persona_id": "reader1", "model": "model-a"},
        "vote": "up", "reason": "",
    }])

    tdir, idir = TMP / "conflict_tasks", TMP / "conflict_inbox"
    for d in (tdir, idir):
        if d.exists():
            shutil.rmtree(d)
        d.mkdir(parents=True)
    task = {"poem_id": poem["id"], "target_read_id": "r-600",
            "voter": {"persona_id": "reader1", "model": None}, "prompt": "..."}
    (tdir / "task-001.json").write_text(json.dumps(task, ensure_ascii=False), encoding="utf-8")
    response = idir / "task-001.response.json"
    response.write_text(json.dumps({"model": "model-b", "vote": "down", "reason": "改判"},
                                   ensure_ascii=False), encoding="utf-8")
    ...
    try:
        PV.cmd_collect(_A)
        assert False, "冲突票必须拒收"
    except ValueError:
        pass
    assert response.exists(), "落盘失败时回执必须留在 inbox，不能提前归档"
    assert not (idir / "ingested" / response.name).exists()
    assert len(R.load_comment_votes()) == 1
    print("[ok] collect 冲突时不落票、不提前移动回执")


def test_batch_collect_rejects_incomplete_ballot():
    poem = _setup_corpus()
    R.VOTES_DIR = TMP
    R.VOTES = TMP / "votes_incomplete.jsonl"
    R.VOTES_VOID = TMP / "void_incomplete.json"
    R.DATA_LOCK = TMP / ".write.lock"
    for p in (R.VOTES, R.VOTES_VOID):
        if p.exists():
            p.unlink()
    tdir, idir = TMP / "incomplete_tasks", TMP / "incomplete_inbox"
    for d in (tdir, idir):
        if d.exists():
            shutil.rmtree(d)
        d.mkdir(parents=True)
    task = {"poem_id": poem["id"],
            "targets": [{"read_id": "r-700"}, {"read_id": "r-701"}],
            "voter": {"persona_id": "reader1", "model": None}, "prompt": "..."}
    (tdir / "task-001.json").write_text(json.dumps(task, ensure_ascii=False), encoding="utf-8")
    response = idir / "task-001.response.json"
    response.write_text(json.dumps({"model": "m", "votes": [
        {"read_id": "r-700", "vote": "up", "reason": ""}
    ], "best": None}, ensure_ascii=False), encoding="utf-8")
    ...
    PV.cmd_collect(_A)
    assert R.load_comment_votes() == []
    assert response.exists(), "票面缺项时整份拒收，回执留给重跑"
    print("[ok] batch 票面缺项整份拒收")
...
if __name__ == "__main__":
    ...
    test_append_is_idempotent_and_conflicts_are_atomic()
    ...
    test_collect_conflict_keeps_response()
    test_batch_collect_rejects_incomplete_ballot()
