# -*- coding: utf-8 -*-
"""参考样例（历史脚本，非通用工具）：把第二批笔记同步产物增量并入 corpus/诗稿.json。

这是一个设备来源增量并入第二批语料时使用的一次性脚本模板，
`NEW_RAW`/`SKIP_GUIDS` 是需要使用者自行填写的参数，不是长期运行的
...
NEW_RAW = ROOT / "corpus" / "raw" / "device-b" / "preprocessed.json"
SKIP_GUIDS = set()  # 仅填入已由作者确认重复、需要跳过的来源记录 ID
