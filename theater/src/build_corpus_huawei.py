# -*- coding: utf-8 -*-
"""参考样例（历史脚本，非通用工具）：把设备备忘录导出合并为 corpus/诗稿.json（01 冻结 schema）。

这是一个设备备忘录导出的一次性入库脚本模板，字段名和文件路径只是示例，
不是"任意设备 → 诗稿.json"的通用转换器。其他来源请照着这个模式写等价脚本，
...
RAW_FILES = [
    RAW / "device-a" / "poems.json",
    RAW / "device-a" / "prose.json",
]
