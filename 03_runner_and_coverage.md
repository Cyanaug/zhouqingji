...
- `api`：直连各家 API。
- `codex-subagent` / `cc-subagent`：使用项目原生角色完成小批量独立任务。
- `agy-subagent` / `codebuddy-cli`：每 task 一个全新无状态进程；优先启用当前 CLI 的 JSON schema/JSON 输出。
- `free-model`：免费模型（会拉低质量，按 `transport` 字段可事后过滤）。

换传输层只改这一层，schema 一行不动。

并发数不是项目常量：Codex、Claude Code 与外部 CLI 的槽位、限流和额度都会变。
派发方按当前环境分波，给主代理保留质检/collect 能力；不能把某次会话的“4 个槽位”
写成 Codex 的永久产品限制。

## 写入一致性（v1.6）

- reads、votes、thread sidecar 共用跨进程写锁，覆盖“读取现状 → 分配 ID → 追加”整个事务。
- 投票身份按 `票种/来源/target/persona` 判定；主动与顺势分流，model 只记出处、不制造第二票。
- 同值重试幂等跳过，冲突整批拒收；best 额外以稳定 `ballot_id` 区分不同票面。
- 历史纠错使用 void sidecar，不删除原始 JSONL。写入后运行 `audit_data.py`。
